...
result_dir <- file.path(project_dir, "results", "editorial_strengthening")
figure_dir <- file.path(project_dir, "figures", "editorial_strengthening")
...
write_tsv_base(all_scores, file.path(result_dir, "editorial_strengthening_scores.tsv"))
...
summary_lines <- c(
  "# Editorial Strengthening Analysis Summary",
  "",
...
writeLines(summary_lines, file.path(result_dir, "editorial_strengthening_analysis_summary.md"), useBytes = TRUE)

panel_label <- function(label, x = 0.018, y = 0.040) {
  fig <- graphics::par("fig")
  x_ndc <- fig[1] + diff(fig[1:2]) * x
  y_ndc <- fig[4] - diff(fig[3:4]) * y
  graphics::text(
    graphics::grconvertX(x_ndc, from = "ndc", to = "user"),
    graphics::grconvertY(y_ndc, from = "ndc", to = "user"),
    label,
    adj = c(0, 1),
    font = 2,
    cex = 1.18,
...
draw_forest <- function(dat, title, score_order = NULL) {
  ...
  graphics::plot(log(dat$estimate), y, xlim = xlim, ylim = c(0.5, length(y) + 1.35),
                 yaxt = "n", ylab = "", xlab = "Log odds ratio per 1 SD score",
                 pch = 19, col = "#1f4e79", main = "")
  graphics::mtext(title, side = 3, line = 0.55, adj = 0.5, font = 2, cex = 0.95)
...
draw_adjusted <- function(dat) {
  ...
  graphics::plot(log(dat$estimate), y, xlim = xlim, ylim = c(0.5, length(y) + 1.35),
                 ...
                 pch = 19, col = "#7c2d12", main = "")
  graphics::mtext("Primary score sensitivity models", side = 3, line = 0.55, adj = 0.5, font = 2, cex = 0.95)
...
draw_bootstrap <- function(dat) {
  ...
  graphics::boxplot(log(dat$estimate_or) ~ groups, col = "#d8e2dc", border = "#264653",
                    ...
                    main = "", cex.axis = 0.65, ylim = range(log(dat$estimate_or), finite = TRUE) + c(0, 0.55))
  graphics::mtext("Bootstrap stability in small cohorts", side = 3, line = 0.55, adj = 0.5, font = 2, cex = 0.95)
...
draw_loo <- function(dat) {
  ...
  graphics::plot(NA, xlim = c(0.5, length(groups) + 0.5),
                 ylim = range(log(dat$estimate), finite = TRUE) + c(-0.2, 0.5),
                 xaxt = "n", xlab = "", ylab = "Leave-one-out log odds ratio",
                 main = "")
  graphics::mtext("Single-sample deletion influence", side = 3, line = 0.55, adj = 0.5, font = 2, cex = 0.95)
  graphics::abline(h = 0, lty = 2, col = "grey50")
...
draw_correlation_heatmap <- function(corr_df, accession = "GSE162694") {
  ...
  graphics::par(mar = c(7.5, 7.8, 4.2, 1.5))
  ...
  graphics::image(seq_len(ncol(mat)), seq_len(nrow(mat)), t(mat[nrow(mat):1, ]),
                  ...
                  xlab = "", ylab = "", main = "")
  graphics::mtext(paste0(accession, " score/proxy correlations"), side = 3, line = 0.55, adj = 0.5, font = 2, cex = 0.95)
...
for (device in c("png", "pdf")) {
  ...
  graphics::par(mar = c(4.2, 9.8, 4.2, 1.1), cex = 0.85)
  ...
  panel_label("B")
  graphics::par(mar = c(5.2, 4.5, 4.2, 1.1), cex = 0.85)
  draw_bootstrap(all_bootstrap)
  panel_label("C")
  graphics::par(mar = c(5.0, 4.5, 4.2, 1.1), cex = 0.85)
  draw_loo(all_loo)
  panel_label("D")
...
figure7_manifest <- data.frame(
  ...
  panel = c("A", "B", "C", "D", "E"),
  source_figure = c(
    paste0("figures/editorial_strengthening/", figure7_stem, ".png"),
    paste0("figures/editorial_strengthening/", figure7_stem, ".png"),
    paste0("figures/editorial_strengthening/", figure7_stem, ".png"),
    paste0("figures/editorial_strengthening/", figure7_stem, ".png"),
    paste0("figures/editorial_strengthening/", figure7_stem, ".png")
  ...
  source_data = c(
    "results/editorial_strengthening/fibrosis_stromal_benchmark_results.tsv",
    "results/editorial_strengthening/bulk_confounding_sensitivity_models.tsv",
    "results/editorial_strengthening/small_sample_bootstrap_iterations.tsv; results/editorial_strengthening/small_sample_bootstrap_stability_summary.tsv",
    "results/editorial_strengthening/small_sample_leave_one_out_models.tsv; results/editorial_strengthening/small_sample_leave_one_out_summary.tsv",
    "results/editorial_strengthening/bulk_score_proxy_correlation_matrix.tsv"
  ),
  generating_script = rep("scripts/22_editorial_strengthening_analyses.R", 5),
  role = c(
...
legacy_png <- file.path(figure_dir, "Figure_7_editorial_strengthening_benchmark_sensitivity.png")
legacy_pdf <- file.path(figure_dir, "Figure_7_editorial_strengthening_benchmark_sensitivity.pdf")
legacy_caption <- file.path(figure_dir, "Figure_7_editorial_strengthening_caption.tsv")
